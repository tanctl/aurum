use relayer::{Config, BlockchainClient};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("current blocks - sepolia: {}, base: {}", sepolia_block, base_block);
    ...
    match client.get_subscription(subscription_id, "sepolia").await? {
        Some(subscription) => {
            ...
            let balance = client.check_balance(subscription.subscriber, "sepolia").await?;
            println!("  subscriber balance: {}", balance);
            
            let has_allowance = client.check_allowance(
