...
fn setup_test_env() {
    env::set_var("DATABASE_URL", "postgresql://relayer:dev_pass@localhost:5432/relayer_test");
    env::set_var("ETHEREUM_RPC_URL", "http://127.0.0.1:8545");
    env::set_var("BASE_RPC_URL", "http://127.0.0.1:8546");
    env::set_var("RELAYER_PRIVATE_KEY", "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80");
    env::set_var("RELAYER_ADDRESS", "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266");
    env::set_var("SUBSCRIPTION_MANAGER_ADDRESS_SEPOLIA", "0x5fbdb2315678afecb367f032d93f642f64180aa3");
    env::set_var("SUBSCRIPTION_MANAGER_ADDRESS_BASE", "0xe7f1725e7734ce288f8367e1bb143e90bb3f0512");
    env::set_var("PYUSD_ADDRESS_SEPOLIA", "0x9fe46736679d2d9a65f0992f2272de9f3c7fa6e0");
    env::set_var("PYUSD_ADDRESS_BASE", "0xcf7ed3acca5a467e9e704c703e8d87f634fb0fc9");
}
...
async fn test_blockchain_client_initialization() {
    ...
    match result {
        ...
        Err(e) => {
            println!("blockchain client initialization failed (expected without anvil): {}", e);
        }
    }
}
...
async fn test_config_validation_for_blockchain() {
    ...
    assert!(config.subscription_manager_address_sepolia.starts_with("0x"));
    ...
    assert_eq!(config.subscription_manager_address_for_chain("sepolia").unwrap(), &config.subscription_manager_address_sepolia);
    assert_eq!(config.subscription_manager_address_for_chain("base").unwrap(), &config.subscription_manager_address_base);
    
    assert_eq!(config.pyusd_address_for_chain("sepolia").unwrap(), &config.pyusd_address_sepolia);
    assert_eq!(config.pyusd_address_for_chain("base").unwrap(), &config.pyusd_address_base);
    
    assert!(config.subscription_manager_address_for_chain("invalid").is_err());
...
async fn test_blockchain_operations_with_anvil() {
    ...
    let config = Config::from_env().expect("should load config");
    let client = BlockchainClient::new(&config).await.expect("should connect to anvil");
    ...
    let sepolia_block = client.get_current_block_number("sepolia").await.expect("should get sepolia block");
    assert!(sepolia_block > 0, "should have blocks on sepolia");
    
    let base_block = client.get_current_block_number("base").await.expect("should get base block");
    assert!(base_block > 0, "should have blocks on base");
    
    println!("sepolia block: {}, base block: {}", sepolia_block, base_block);
}
...
async fn test_blockchain_error_handling() {
    ...
    assert!(config_result.is_err(), "should fail with invalid private key");
}